...
export const expenseReducer = createReducer(initialState, (builder) => {
  ...
  builder.addCase(DELETE_EXPENSE_REQUEST, (state, action) => {
    return {
      loading: true,
      expensesData: [],
    };
  });
  builder.addCase(DELETE_EXPENSE_SUCCESS, (state, action) => {
    return {
      loading: false,
      expensesData: action.payload,
    };
  });
  builder.addCase(DELETE_EXPENSE_FAIL, (state, action) => {
    return {
      loading: false,
      error: action.payload,
    };
