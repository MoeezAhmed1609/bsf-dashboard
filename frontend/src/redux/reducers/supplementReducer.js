...
export const supplementReducer = createReducer(initialState.supplements, (builder) => {
...
export const supplementsSalesReducer = createReducer(
  ...
  (builder) => {
    ...
    builder.addCase(UPDATE_UNPAID_SUPPLEMENTS_SALES_REQUEST, (state, action) => {
      return {
        loading: true,
        ...state,
      }
    })
    builder.addCase(UPDATE_UNPAID_SUPPLEMENTS_SALES_SUCCESS, (state, action) => {
      return {
        loading: false,
        success: action.payload.success,
        supplementsData: action.payload.sales,
